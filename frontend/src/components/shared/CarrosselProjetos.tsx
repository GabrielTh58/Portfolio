...
export default function CarrosselProjetos({imagens, idProximoProjeto, idProjetoAnterior}: CarrosselImagensProps) {
	...
	return (
		<Carousel opts={{ loop: true }} className="w-full md:w-11/12 xl:w-full">
			<CarouselContent>
				{imagens.map((imagem) => (
					<CarouselItem key={imagem} className="relative w-full aspect-[16/9] md:aspect-[16/10] sm:h-80 md:h-72">
						<Image src={imagem} alt="Imagem" fill className="object-cover" />
					</CarouselItem>
				))}
			</CarouselContent>
			<div className="flex items-center gap-2 mt-8 md:mt-0">
				<Button onClick={navegarParaProjetoAnterior} />
				<Button proximo onClick={navegarParaProximoProjeto} />
			</div>
		</Carousel>
	)
}
...
function Button({onClick, proximo=false}: ButtonProps ){
	return(
		<button
			onClick={onClick}
			className={`md:absolute z-20 cursor-pointer border rounded-full p-1 transition-all ease duration-300 hover:scale-105 
				hover:bg-slate-950/80 hover:shadow-[0_20px_40px_rgba(25,60,184,0.18)]				
